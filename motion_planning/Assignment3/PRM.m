...
for i = 2:nsamples
    ...
    %
    [distances_sorted, distances_sorted_i] = sort(distances);
    for ii=1:min(k, size(distances,2))
        if LocalPlanner(x, samples(:,distances_sorted_i(ii)))
            % There is straight line path between x and iith point from k-nearest
            % neighbors => update the tree
            nedges = nedges + 1;
            edges(nedges, :) = [i distances_sorted_i(ii)];
            edge_lengths(nedges) = distances_sorted(ii);
        end
    end
